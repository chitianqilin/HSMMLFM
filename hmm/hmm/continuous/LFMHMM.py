    ...
    def unpack_params(self, params_array):
        ...
        for i in range(self.n):
            ...
            for j in range(self.number_outputs):
                ...
                idx += 1
            for j in range(self.number_outputs):
                for k in range(self.number_latent_f):
                    sensi[i][j][k] = params_array[idx]
                    idx += 1
            for j in range(self.number_latent_f):
    ...
    def _update_emission_params(self, lfms_params):
        ...
        for i in range(self.n):
            ...
            if self._INDEPENDENT_OUTPUTS:
                ...
                assert self.number_outputs == self.number_latent_f
                for d in range(self.number_outputs):
                    for q in range(self.number_latent_f):
